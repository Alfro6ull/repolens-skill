...
3. Generate an evidence-backed performance report:

```bash
node repolens-perf/scripts/perf_report.mjs /path/to/repo "/activity/:id"
```

4. Read only the reference material needed for the task:
...
Run `trace_module.mjs` with a specific target. Use `--hops 3` for route-to-component tracing, and rely on `perf_report.mjs` default depth for route performance reports. Use the report as the first draft and improve it with code reading when needed.
